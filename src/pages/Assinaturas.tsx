...
import { fetchSignatures } from '@/data/api'
import { useQuery } from '@/hooks/useQuery'
import { signatureTone } from '@/lib/status'
import { formatDate } from '@/lib/format'

export default function Assinaturas() {
  const { data, loading, error, reload } = useQuery(fetchSignatures)
  const signatures = useMemo(() => data ?? [], [data])
  ...
  return (
    <>
      ...
      <div className="mb-6 grid grid-cols-4 gap-4">
        <Stat label="Total de signatários" value={loading ? '—' : signatures.length} />
        <Stat label="Assinados" value={loading ? '—' : signed} hintTone="success" />
        <Stat label="Aguardando" value={loading ? '—' : awaiting.length} hintTone="warning" />
        <Stat label="Negócios em assinatura" value={loading ? '—' : dealsInSigning} />
      </div>
